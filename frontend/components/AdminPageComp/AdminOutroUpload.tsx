...
async function uploadToGCS(
  ...
  const sigRes = await userRequest.post("/pams-studio/outro-scenes/upload-signature", {
...
// ─── Existing Outro Scene Row ─────────────────────────────────────────────────
function OutroSceneRow({
  scene,
  onDelete,
}: {
  scene: OutroScene;
...
// ─── Main Component ───────────────────────────────────────────────────────────
export default function AdminOutroUpload() {
  // List
  const [scenes, setScenes]       = useState<OutroScene[]>([]);
  ...
  // ── Fetch existing outro scenes ────────────────────────────────────────────
  const fetchScenes = async () => {
    try {
      const res = await userRequest.get("/pams-studio/outro-scenes");
      setScenes(res.data.outroScenes ?? []);
    } catch {
      toast.error("Failed to load outro scenes");
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    if (detectedDur > 10) {
      toast.error(`Outro must be ≤ 10s (detected ${detectedDur.toFixed(1)}s)`);
      return;
    }
    ...
    try {
      ...
      // 3. Save to DB
      setUploadStage("Saving outro scene…");
      await userRequest.post("/pams-studio/outro-scenes", {
        title:        title.trim(),
        videoUrl,
        publicId,
        thumbnailUrl,
      ...
      toast.success(`"${title.trim()}" outro scene added!`);
      resetForm();
      await fetchScenes();
    } catch (err: unknown) {
  ...
  const handleDelete = async (id: string) => {
    try {
      await userRequest.delete(`/pams-studio/outro-scenes/${id}`);
      toast.success("Outro scene removed");
      setScenes((prev) => prev.filter((s) => s._id !== id));
    } catch {
      toast.error("Failed to delete outro scene");
    }
  };
        ...
        <div className="grid grid-cols-[1fr_120px] gap-3">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-white/5 border border-white/10 text-white placeholder-white/20 rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-cyan-400/40 transition-colors"
            />
          </div>
          <div className="space-y-1.5">
            <label className="text-[10px] font-semibold text-white/40 uppercase tracking-widest">
            ...
            <input
              ...
              min={0}
              className="w-full bg-white/5 border border-white/10 text-white rounded-xl px-3 py-2 text-sm focus:outline-none focus:border-cyan-400/40 transition-colors"
            />
          </div>
        </div>
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    ...
                    {detectedDur !== null && (
                      <>
                        ...
                        <span className={`text-[11px] font-semibold ${detectedDur > 10 ? "text-red-400" : "text-cyan-300"}`}>
          ...
          <button
            ...
            className={`flex items-center gap-2 px-5 py-2.5 rounded-xl text-xs font-bold uppercase tracking-wider transition-all ${
              uploading || !videoFile || !title.trim() || !detectedDur || detectedDur > 10
                ...
                : "bg-gradient-to-r from-cyan-500/80 to-teal-500/80 border border-cyan-400/40 text-white hover:from-cyan-400/80 hover:to-teal-400/80 shadow-[0_0_18px_rgba(34,211,238,0.25)]"
            }`}
          >
            {uploading ? (
              <>
      ...
      <div className="rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl p-6 space-y-4">
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-10 rounded-2xl border border-dashed border-white/8 gap-2">
            ...
            <p className="text-xs text-white/25">No outro scenes yet — upload one above</p>
          </div>
        ) : (
